...
def purchase_cursor(amount):
    ...
    if amount.lower() == "max":
        clear()
        while(current_cookies >= cost_cursor):
            current_cookies -= float(cost_cursor)
            num_cursor += 1
            cost_cursor = 15 * (1.15 ** num_cursor)
    elif int(amount) >= 1:
        amount = int(amount)
        clear()
        if current_cookies >= cost_cursor:
            while(amount >= 1 and current_cookies >= cost_cursor):
                current_cookies -= float(cost_cursor)
                num_cursor += 1
                cost_cursor = 15 * (1.15 ** num_cursor)
                amount -= 1
        else:
            print("You dont have enough cookies to buy [%d] cursor(s)." % amount)
    else:
        clear()
        print("amount error")
    print("You now have [%d] cursor(s)\n" % num_cursor)

def purchase_grandma(amount):
    global current_cookies, num_grandma, cost_grandma
    if amount.lower() == "max":
        clear()
        while(current_cookies >= cost_grandma):
            current_cookies -= float(cost_grandma)
            num_grandma += 1
            cost_grandma = 100 * (1.15 ** num_grandma)
    elif int(amount) >= 1:
        amount = int(amount)
        clear()
        if current_cookies >= cost_grandma:
            while(amount >= 1 and current_cookies >= cost_grandma):
                current_cookies -= float(cost_grandma)
                num_grandma += 1
                cost_grandma = 100 * (1.15 ** num_grandma)
                amount -= 1
        else:
            print("You dont have enough cookies to buy [%d] grandma(s)." % amount)
    else:
        clear()
        print("amount error")
    print("You now have [%d] grandma(s)\n" % num_grandma)

def purchase_farm(amount):
    global current_cookies, num_farm, cost_farm
    if amount.lower() == "max":
        clear()
        while(current_cookies >= cost_farm):
            current_cookies -= float(cost_farm)
            num_farm += 1
            cost_farm = 1100 * (1.15 ** num_farm)
    elif int(amount) >= 1:
        amount = int(amount)
        clear()
        if current_cookies >= cost_farm:
            while(amount >= 1 and current_cookies >= cost_farm):
                current_cookies -= float(cost_farm)
                num_farm += 1
                cost_farm = 1100 * (1.15 ** num_farm)
                amount -= 1
        else:
            print("You dont have enough cookies to buy [%d] farm(s)." % amount)
    else:
        clear()
        print("amount error")
    print("You now have [%d] farm(s)\n" % num_farm)

def purchase_mine(amount):
    global current_cookies, num_mine, cost_mine
    if amount.lower() == "max":
        clear()
        while(current_cookies >= cost_mine):
            current_cookies -= float(cost_mine)
            num_mine += 1
            cost_mine = 12000 * (1.15 ** num_mine)
    elif int(amount) >= 1:
        amount = int(amount)
        clear()
        if current_cookies >= cost_mine:
            while(amount >= 1 and current_cookies >= cost_mine):
                current_cookies -= float(cost_mine)
                num_mine += 1
                cost_mine = 12000 * (1.15 ** num_mine)
                amount -= 1
        else:
            print("You dont have enough cookies to buy [%d] mine(s)." % amount)
    else:
        clear()
        print("amount error")
    print("You now have [%d] mine(s)\n" % num_mine)

def purchase_factory(amount):
    global current_cookies, num_factory, cost_factory
    if amount.lower() == "max":
        clear()
        while(current_cookies >= cost_factory):
            current_cookies -= float(cost_factory)
            num_factory += 1
            cost_factory = 130000 * (1.15 ** num_factory)
    elif int(amount) >= 1:
        amount = int(amount)
        clear()
        if current_cookies >= cost_factory:
            while(amount >= 1 and current_cookies >= cost_factory):
                current_cookies -= float(cost_factory)
                num_factory += 1
                cost_factory = 130000 * (1.15 ** num_factory)
                amount -= 1
        else:
            print("You dont have enough cookies to buy [%d] factory(s)." % amount)
    else:
        clear()
        print("amount error")
    print("You now have [%d] factory(s)\n" % num_factory)

def purchase_bank(amount):
    global current_cookies, num_bank, cost_bank
    if amount.lower() == "max":
        clear()
        while(current_cookies >= cost_bank):
            current_cookies -= float(cost_bank)
            num_bank += 1
            cost_bank = 1400000 * (1.15 ** num_bank)
    elif int(amount) >= 1:
        amount = int(amount)
        clear()
        if current_cookies >= cost_bank:
            while(amount >= 1 and current_cookies >= cost_bank):
                current_cookies -= float(cost_bank)
                num_bank += 1
                cost_bank = 1400000 * (1.15 ** num_bank)
                amount -= 1
        else:
            print("You dont have enough cookies to buy [%d] bank(s)." % amount)
    else:
        clear()
        print("amount error")
    print("You now have [%d] bank(s)\n" % num_bank)

def purchase_temple(amount):
    global current_cookies, num_temple, cost_temple
    if amount.lower() == "max":
        clear()
        while(current_cookies >= cost_temple):
            current_cookies -= float(cost_temple)
            num_temple += 1
            cost_temple = 20000000 * (1.15 ** num_temple)
    elif int(amount) >= 1:
        amount = int(amount)
        clear()
        if current_cookies >= cost_temple:
            while(amount >= 1 and current_cookies >= cost_temple):
                current_cookies -= float(cost_temple)
                num_temple += 1
                cost_temple = 20000000 * (1.15 ** num_temple)
                amount -= 1
        else:
            print("You dont have enough cookies to buy [%d] temple(s)." % amount)
    else:
        clear()
        print("amount error")
    print("You now have [%d] temple(s)\n" % num_temple)

def purchase_wiz(amount):
    global current_cookies, num_wiz, cost_wiz
    if amount.lower() == "max":
        clear()
        while(current_cookies >= cost_wiz):
            current_cookies -= float(cost_wiz)
            num_wiz += 1
            cost_wiz = 330000000 * (1.15 ** num_wiz)
    elif int(amount) >= 1:
        amount = int(amount)
        clear()
        if current_cookies >= cost_wiz:
            while(amount >= 1 and current_cookies >= cost_wiz):
                current_cookies -= float(cost_wiz)
                num_wiz += 1
                cost_wiz = 330000000 * (1.15 ** num_wiz)
                amount -= 1
        else:
            print("You dont have enough cookies to buy [%d] wizard tower(s)." % amount)
    else:
        clear()
        print("amount error")
    print("You now have [%d] wizard tower(s)\n" % num_wiz)

def purchase_ship(amount):
    global current_cookies, num_ship, cost_ship
    if amount.lower() == "max":
        clear()
        while(current_cookies >= cost_ship):
            current_cookies -= float(cost_ship)
            num_ship += 1
            cost_ship = 5100000000 * (1.15 ** num_ship)
    elif int(amount) >= 1:
        amount = int(amount)
        clear()
        if current_cookies >= cost_ship:
            while(amount >= 1 and current_cookies >= cost_ship):
                current_cookies -= float(cost_ship)
                num_ship += 1
                cost_ship = 5100000000 * (1.15 ** num_ship)
                amount -= 1
        else:
            print("You dont have enough cookies to buy [%d] shipment(s)." % amount)
    else:
        clear()
        print("amount error")
    print("You now have [%d] shipment(s)\n" % num_ship)

def purchase_alc(amount):
    global current_cookies, num_alc, cost_alc
    if amount.lower() == "max":
        clear()
        while(current_cookies >= cost_alc):
            current_cookies -= float(cost_alc)
            num_alc += 1
            cost_alc = 75000000000 * (1.15 ** num_alc)
    elif int(amount) >= 1:
        amount = int(amount)
        clear()
        if current_cookies >= cost_alc:
            while(amount >= 1 and current_cookies >= cost_alc):
                current_cookies -= float(cost_alc)
                num_alc += 1
                cost_alc = 75000000000 * (1.15 ** num_alc)
                amount -= 1
        else:
            print("You dont have enough cookies to buy [%d] alchemy lab(s)." % amount)
    else:
        clear()
        print("amount error")
    print("You now have [%d] alchemy lab(s)\n" % num_alc)

def purchase_port(amount):
    global current_cookies, num_port, cost_port
    if amount.lower() == "max":
        clear()
        while(current_cookies >= cost_port):
            current_cookies -= float(cost_port)
            num_port += 1
            cost_port = 1000000000000 * (1.15 ** num_port)
    elif int(amount) >= 1:
        amount = int(amount)
        clear()
        if current_cookies >= cost_port:
            while(amount >= 1 and current_cookies >= cost_port):
                current_cookies -= float(cost_port)
                num_port += 1
                cost_port = 1000000000000 * (1.15 ** num_port)
                amount -= 1
        else:
            print("You dont have enough cookies to buy [%d] portal(s)." % amount)
    else:
        clear()
        print("amount error")
    print("You now have [%d] portal(s)\n" % num_port)

def purchase_tima(amount):
    global current_cookies, num_tima, cost_tima
    if amount.lower() == "max":
        clear()
        while(current_cookies >= cost_tima):
            current_cookies -= float(cost_tima)
            num_tima += 1
            cost_tima = 14000000000000 * (1.15 ** num_tima)
    elif int(amount) >= 1:
        amount = int(amount)
        clear()
        if current_cookies >= cost_tima:
            while(amount >= 1 and current_cookies >= cost_tima):
                current_cookies -= float(cost_tima)
                num_tima += 1
                cost_tima = 14000000000000 * (1.15 ** num_tima)
                amount -= 1
        else:
            print("You dont have enough cookies to buy [%d] time machine(s)." % amount)
    else:
        clear()
        print("amount error")
    print("You now have [%d] time machine(s)\n" % num_tima)
